  ...
  <layout>
    <div class="table-responsive-lg">
      ...
      >
        ...
        <tbody>
          <tr v-for="posto in postos" :key="posto.id" :value="posto.id">
            <td>{{ posto.nome_posto }}</td>
            <td align="center">
              <Link :href="'/posto/visualizar/' + posto.id"
                ><i class="fas fa-eye"></i
              ></Link>
              <Link :href="'/posto/editar/' + posto.id"
                ><i class="fas fa-edit"></i
              ></Link>              
              <span
                ><i
                  class="fas fa-trash-alt"
                  @click="sendForm(posto.id)"
                ></i
              ></span>
            </td>
          </tr>
    ...
    sendForm(id) {
      ...
      bootbox.confirm({
        ...
        message:
          ...
          "<span style='font-weight:bold; position: relative; top: 5px;'>Deletar posto?</span>",
        buttons: {
          cancel: {
        ...
        callback: function (result) {
          if (result == true) {
            v.$inertia.post("/posto/deletar/"+id, {
              forceFormData: true,
